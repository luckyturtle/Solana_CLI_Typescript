...
export const updateNFTMetadata = async (
  ...
  log.info('=> UpdateNFTMetadata Func is Processing');
  // return;

  const mintKey = new PublicKey(nftAddress);
  log.info(`--> mintKey:`);
  log.info(mintKey.toBase58());
  const instructions: TransactionInstruction[] = [];
  ...
  log.info(`--> userTokenAccoutAddress:`);
  log.info(userTokenAccoutAddress.toBase58());
  
  // Get metadata
  const metadataAccount = await getMetadata(mintKey);
  
  log.info(`--> metadataAccount:`);
  log.info(metadataAccount.toBase58());
  const users = [
    {address: wallet.publicKey.toBase58(), share: 100}
  ];
  const creators = users.map(
    creator =>
      new Creator({
        address: creator.address,
        share: creator.share,
  ...
  // Force wait for max confirmations
  await connection.getParsedConfirmedTransaction(res.txid, 'confirmed');
  log.info('=> NFT Metadata updated', res.txid);
} catch (e) {
